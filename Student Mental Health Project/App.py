...
def predict():
    ...
    if submit_button:
        ...
        if prob < 0.3:
            ...
            st.info(f"🟡 Depression Risk Score: **{prob:.2f}**")
        else:
            st.error(f"🔴 Depression Risk Score: **{prob:.2f}**")

        
 
        st.warning(f"{emoji} Depression Risk: {prob*100:.2f}%")
        ...
        if gender == "Female" and age < 25:
            suggestions.append("👩‍🎓 As a young one balancing studies and personal growth, consider mentoring programs or peer support groups that could provide guidance and connection.")
...
def feedback_form():
    ...
    # Suggestions
    suggestions = st.text_area("Feedback / Any suggestions to improve the app?")
...
def interactive_dashboard():
    ...
    try:
        ...
        st.error("Dataset file 'cleaned_data.xlx' not found. Please ensure the file is in the correct location.")
    except Exception as e:
...
def main():
    ...
    app_mode = st.sidebar.radio("Navigation", ["🏠 Home", "🔮 Prediction", "📊 Analytics", "📚 Resources", "📋 Feedback", "ℹ️ About"])
    st.sidebar.image("D:\Student Mental Health Project\Img1.jpg", caption="Mental Health Awareness", use_container_width=True)
...
# Left column for the image
        with col2:
            st.image("D:\Student Mental Health Project\Img2.jpg",caption="It's okay to take a moment.", width=500)
...
# Right column for the text
    ...
    - **Email**: mentalhealthsupport@example.com
    - **Phone**: +91 2345678901
    - **Website**: [www.mentalhealthsupport.com](http://www.example.com)
    """)
